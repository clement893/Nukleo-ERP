...
async def chat_completion(
    ...
    try:
        ...
        try:
            ...
            if erp_context:
                system_prompt += f"\n\n=== DONNÉES ERP DISPONIBLES ===\n{erp_context}\n=== FIN DES DONNÉES ===\n\n⚠️ INSTRUCTIONS CRITIQUES ⚠️\n\nTu as accès aux données RÉELLES de l'ERP Nukleo ci-dessus. Ces données sont EXACTES et à jour.\n\nRÈGLES ABSOLUES:\n1. Si l'utilisateur demande des informations sur une personne, une entreprise, un projet, une opportunité ou un employé, tu DOIS utiliser UNIQUEMENT les données fournies ci-dessus.\n2. Ne donne JAMAIS de réponses génériques comme 'Je n'ai pas accès à vos données' ou 'Les infos ne sont pas disponibles'.\n3. Si des données existent dans le contexte ERP ci-dessus, utilise-les pour répondre de manière précise et détaillée.\n4. Si l'utilisateur demande 'combien de clients avons-nous' et que le contexte contient des entreprises marquées comme clients, COMPTE-les et donne le nombre exact.\n5. Si l'utilisateur demande 'qui sont nos employés' et que le contexte contient des employés, LISTE-les tous.\n6. Si l'utilisateur demande 'connais-tu nos contacts' et que le contexte contient des contacts, confirme que tu les connais et donne des exemples.\n7. Si le contexte ERP ci-dessus est VIDE (pas de données listées), cela signifie que j'ai cherché dans la base de données mais n'ai rien trouvé. Dans ce cas, dis clairement: 'J'ai cherché dans vos données ERP mais je n'ai pas trouvé d'informations correspondantes.'\n8. Ne mentionne JAMAIS que tu n'as pas accès aux données. Tu as toujours accès, mais parfois les données n'existent pas dans la base.\n\nExemple: Si l'utilisateur demande 'combien de clients avons-nous' et que le contexte ERP contient 10 entreprises marquées comme clients, réponds: 'Vous avez 10 clients.'"
            else:
