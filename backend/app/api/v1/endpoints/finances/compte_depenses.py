...
async def update_compte_depenses(
    ...
    logger.debug(f"[update_compte_depenses] Reloaded account {account.id} from DB, status={account.status}")
    ...
    logger.debug(f"[update_compte_depenses] Loaded relationships for account {account.id}: employee={account_employee is not None}, reviewer={account_reviewer is not None}")
    ...
    logger.debug(f"[update_compte_depenses] Returning account dict with status={account_dict['status']}")
    return ExpenseAccountResponse(**account_dict)
