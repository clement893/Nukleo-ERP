...
function ManagementCompteDepensesContent() {
  ...
  const pendingCount = useMemo(() => {
    return expenseAccounts.filter(acc => 
      acc.status === 'submitted' || acc.status === 'under_review'
    ).length;
  }, [expenseAccounts]);
  ...
  return (
    <MotionDiv variant="slideUp" duration="normal" className="space-y-2xl">
      <PageHeader
        title="Validation des comptes de dépenses"
        description={`Gérez les demandes de remboursement des employés${pendingCount > 0 ? ` - ${pendingCount} demande${pendingCount > 1 ? 's' : ''} en attente de validation` : ''}`}
        breadcrumbs={[
          { label: 'Dashboard', href: '/dashboard' },
          { label: 'Module Management', href: '/dashboard/management' },
          { label: 'Comptes de dépenses' },
        ]}
      />
...
export default function ManagementCompteDepensesPage() {
  return <ManagementCompteDepensesContent />;
}
