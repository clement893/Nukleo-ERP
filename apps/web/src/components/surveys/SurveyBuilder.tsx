...
export default function SurveyBuilder({
  ...
  const handleReorderQuestions = useCallback((newOrder: DragDropListItem[]) => {
    const reorderedQuestions = newOrder.map((item) =>
      currentSurvey.questions.find((q) => q.id === item.id)!
    );
    setCurrentSurvey((prev) => ({
      ...prev,
  ...
  return (
    <PageContainer className="py-8">
                  ...
                  content: (
                    <div className="flex items-center justify-between w-full">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="ghost" size="sm" color="danger" onClick={() => handleDeleteQuestion(question.id)}>
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card title={t('survey_status') || 'Survey Status'}>
            <Select
              ...
              onChange={(e) => setCurrentSurvey({ ...currentSurvey, status: e.target.value as Survey['status'] })}
              options={[
      ...
      <Modal
        ...
        title={editingQuestion?.id && currentSurvey.questions.find(q => q.id === editingQuestion.id) ? t('edit_question') || 'Edit Question' : t('add_question') || 'Add Question'}
        size="lg"
      >
        {editingQuestion && (
          <div className="space-y-4">
            <Select
              ...
              onChange={(e) => setEditingQuestion({ ...editingQuestion, type: e.target.value as SurveyQuestionType })}
            ...
            {(editingQuestion.type === 'scale' || editingQuestion.type === 'rating' || editingQuestion.type === 'nps') && (
              <div className="grid grid-cols-3 gap-4">
                <Input
                  label={t('min_value') || 'Min'}
                  type="number"
                  value={editingQuestion.scaleMin || 1}
                  onChange={(e) => setEditingQuestion({ ...editingQuestion, scaleMin: parseInt(e.target.value) || 1 })}
                />
                <Input
                  label={t('max_value') || 'Max'}
                  type="number"
                  value={editingQuestion.scaleMax || 5}
                  onChange={(e) => setEditingQuestion({ ...editingQuestion, scaleMax: parseInt(e.target.value) || 5 })}
                />
                <Input
                  label={t('step') || 'Step'}
                  type="number"
                  value={editingQuestion.scaleStep || 1}
                  onChange={(e) => setEditingQuestion({ ...editingQuestion, scaleStep: parseInt(e.target.value) || 1 })}
                />
              </div>
            )}
            ...
            {currentSurvey.questions.length > 0 && (
              <div className="border-t pt-4">
                <h4 className="text-sm font-semibold mb-2">{t('conditional_logic') || 'Conditional Logic (Show if)'}</h4>
