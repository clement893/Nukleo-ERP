 ...
 * Allows adding, viewing, and managing documents for a company
...
export function CompanyDocuments({ 
  ...
  const loadDocuments = () => {
    try {
      ...
      // In production, you'd want a dedicated API endpoint for company documents
      const storageKey = `company_documents_${companyId}`;
      const stored = localStorage.getItem(storageKey);
      
      if (stored) {
        try {
          const docs = JSON.parse(stored) as CompanyDocument[];
          setDocuments(docs);
        } catch (parseError) {
  ...
  const saveDocuments = async (newDocuments: CompanyDocument[]) => {
    try {
      ...
      {documents.length === 0 ? (
        <Card className="glass-card p-6">
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p>Aucun document pour cette entreprise.</p>
