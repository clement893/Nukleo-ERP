...
export function NukleoSidebar({ 
  ...
  const getCategoryColor = (name: string) => {
    ...
    return colors[name] || 'var(--nukleo-purple)';
  };

  const renderNavItem = (item: NavItem, isInGroup = false) => {
    const active = isActive(item.href);
    const color = isInGroup ? 'var(--nukleo-purple)' : getCategoryColor(item.name);
    ...
    return (
      <Link
        ...
        className={clsx(
          ...
          active
            ? 'glass-card text-primary-500 shadow-sm border border-primary-500/20'
      ...
      >
        {/* Active indicator */}
        {active && !collapsed && (
          <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-10 bg-nukleo-gradient rounded-r-full" />
        )}
        ...
        {!collapsed && (
          <>
            ...
            {item.badge && (
              <span className="px-2 py-0.5 text-xs font-semibold rounded-full bg-primary-500/10 text-primary-500">
                {item.badge}
              </span>
            )}
          </>
        ...
        {collapsed && item.badge && (
          <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-primary-500 text-white text-xs flex items-center justify-center font-semibold">
            {item.badge}
          </div>
        )}
      </Link>
  ...
  const renderNavGroup = (group: NavGroup) => {
    ...
    return (
      <div key={group.name} className="space-y-1">
        ...
        {/* Group items */}
        {isOpen && !collapsed && (
          <div className="ml-3 space-y-1 border-l-2 border-nukleo-lavender/20 pl-3">
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          'fixed top-0 left-0 h-screen bg-background border-r border-nukleo-lavender/20 z-50 transition-all duration-300 flex flex-col',
          collapsed ? 'w-20' : 'w-64',
      ...
      >
        ...
        <div className="relative bg-nukleo-gradient overflow-hidden p-6">
          ...
          {onToggleCollapse && (
            <button
              ...
              className="absolute -right-3 top-1/2 -translate-y-1/2 hidden md:flex w-6 h-6 rounded-full bg-primary-500 text-white items-center justify-center shadow-lg hover:scale-110 transition-transform"
            >
          ...
          <div className="p-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 rounded-xl glass-card border border-nukleo-lavender/20 focus:border-primary-500 focus:outline-none text-sm"
              />
            </div>
          </div>
        )}
        ...
        <div className="p-4 border-t border-nukleo-lavender/20">
            ...
            {!collapsed ? (
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  ...
                  <div className="relative w-10 h-10 rounded-full border-2 border-white/20 overflow-hidden bg-primary-500/20 flex items-center justify-center">
                    <span className="text-primary-500 font-semibold">JT</span>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center gap-2">
                <div className="relative w-10 h-10 rounded-full border-2 border-white/20 overflow-hidden bg-primary-500/20 flex items-center justify-center">
                  <span className="text-primary-500 font-semibold text-xs">JT</span>
                </div>
