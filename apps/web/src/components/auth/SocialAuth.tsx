...
export default function SocialAuth({
  ...
  const handleSignIn = async (provider: SocialProvider) => {
    ...
    try {
      ...
      const baseUrl = process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_DEFAULT_API_URL || 'http://localhost:8000';
      ...
      setError(err instanceof Error ? err.message : `Failed to sign in with ${providerConfigs[provider].name}`);
      setLoading(null);
    }
  };
          ...
          return (
            <Button
              ...
              className={clsx(
                ...
                provider === 'github' && 'border-gray-900 dark:border-border',
